<template>
    <div class="sendMsgCon">
        ...
        <el-tree-select multiple v-model="userVal" show-checkbox :data="treeUserList" placeholder="请选择发送人"
        ...
        <div class="sendMsgCon-content">
            <div class="text">
                <el-input placeholder="请输入内容" v-model="msgContent" style="width: 100%;" :rows="2"
...
watch(() => props.alarmSelect ,(n) => {
    ...
    if(n.alarmLevel == 0) {
        ...
        alarmLevel = '三级'
    } else if(n.alarmLevel == 4) {
        alarmLevel = '四级'
    }
    if(!alarmLevel) return
    msgContent.value = msgContent.value + '，告警等级为' + alarmLevel
                ...
                res.data.map((item: ItemType) => {
                    ...
                    if (item.list && item.list.length > 0) {
                        item.list.map((info: infoType) => {
                            ...
                            obj2.label = info.name,
                                obj2.value = `${info.mobilePhone},${info.name}`
...
function sendMsgBtn() {
    if(userVal.value && userVal.value.length > 0) {
        ...
        userVal.value.map((item : string) => {
            const obj = {receiveName:'',receivePhone:'',content:'',linkInfo:'',linkSys:'',sendName:'',msgId:''}
